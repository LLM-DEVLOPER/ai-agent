...
**深入追问：** 什么是 Chunked Prefill？为什么需要它？

**回答要点：**

Chunked Prefill 是 vLLM 的调度优化技术，将长 prompt 的 prefill 阶段切分成多个小块（chunk），与 decode 请求交替执行，而不是一次性处理完整个 prefill。

为什么需要：长 prompt 的 prefill 是计算密集型，可能需要几秒钟，期间会阻塞所有 decode 请求，导致其他用户的 token 生成卡顿。Chunked Prefill 把长 prefill 拆成多个小块，每处理一个 chunk 就让 decode 请求执行一轮，保证所有用户的响应流畅性。

Trade-off：被切分的 prefill 请求的 TTFT 会略微增加（因为被打断），但整体系统的公平性和用户体验更好。通常 chunk size 设置为 512~2048 tokens。这是高并发场景下保证服务质量的关键技术，vLLM 默认开启。
...
量化把模型权重从 fp16/bf16 压缩到 int8 或 int4，显存减半或减到四分之一，推理速度也有提升（尤其是 weight-only 量化在 decode 阶段收益明显，因为瓶颈就是显存带宽）。代价是精度损失。

**主流量化方法对比：**

| 方法 | 核心机制 | 精度（INT4） | 推理速度 | 适用场景 |
|------|---------|-------------|---------|---------|
| **GPTQ** | 分层近似 + 误差补偿 | 中等 | 快（GPU 优化好） | 服务端推理、vLLM 集成 |
| **AWQ** | 激活感知 + 保护重要权重 | 高（INT4 下最优） | 快 | 精度敏感任务、低比特量化 |
| **GGUF** | CPU 友好格式 + llama.cpp | 中等 | 中（CPU 推理） | 本地部署、边缘设备 |
| **bitsandbytes** | NF4 量化 + 动态反量化 | 高（训练友好） | 慢（动态开销） | QLoRA 微调、单卡训练 |

**面试一句话总结：** GPTQ 和 AWQ 都是大模型后训练量化方法。GPTQ 主要通过分层近似和误差补偿来减少量化损失；AWQ 则引入激活信息，识别并保护对输出更重要的权重，因此通常能在低比特量化下更好地兼顾精度和效率。
...
**深入追问：** 推理服务 OOM 了，怎么排查和解决？

**回答要点：**

OOM（Out of Memory）是推理服务最常见的稳定性问题。排查思路分三步：

**1. 定位显存消耗来源**
- 模型权重（固定）：70B fp16 模型约 140GB
- KV Cache（动态）：`batch_size × seq_len × num_layers × hidden_dim × 2`，高并发时是主要变量
- 激活值（临时）：prefill 阶段的中间激活，长 prompt 时占用显著
- 碎片（隐性）：频繁分配释放导致的显存碎片

**2. 常见 OOM 场景与根因**
- **并发打满**：KV Cache 随并发线性增长，没有设置上限 → 解法：vLLM 配置 `max_num_seqs`、`gpu_memory_utilization=0.9`（留 10% 余量）
- **超长输入/输出**：单请求 `max_tokens` 过大 → 解法：限制 `max_model_len`、`max_tokens`
- **Prefill 峰值**：长 prompt 的 prefill 激活值瞬时占用过高 → 解法：开启 chunked prefill
- **显存碎片**：长时间运行后碎片累积 → 解法：定期重启服务或使用 PagedAttention（vLLM 自带）

**3. 应急与预防**
- 应急：降低并发上限、拒绝超长请求、重启服务清理碎片
- 预防：监控显存水位线（设置 85% 告警）、压测确定并发上限、配置熔断策略

**面试一句话总结**：OOM 排查先看 KV Cache 占用（并发 × 序列长度），再看是否有超长请求或碎片，解法是限制并发数、序列长度，并预留显存余量。
